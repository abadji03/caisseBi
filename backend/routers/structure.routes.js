...
router.post('/',authenticateToken, requireStructureAccess, requirePermission('config.access'), upload.single('logo'), structureController.createStructure);

//PUT: Modification d’une structure (logo en option)
router.put('/:id',authenticateToken, requireStructureAccess, requirePermission('config.access'), upload.single('logo'), structureController.updateStructure);

//DELETE: Suppression d’une structure
router.delete('/:id',authenticateToken, requireStructureAccess, requirePermission('config.access'), structureController.deleteStructure);
...
router.get('/code/:code_structure',authenticateToken, requireStructureAccess, structureController.getStructureByCodeStructure);
...
router.patch('/:id/status',authenticateToken, requireStructureAccess, requirePermission('config.access'), structureController.updateStructureStatus);
