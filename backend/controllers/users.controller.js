...
exports.create = async (req, res) => {
  try {
    ...
    // 🔔 NOTIFICATION : nouvel utilisateur créé
    try {
      await NotificationService.notifierUtilisateurCree(
        user.code_structure || authUser.code_structure,
        user,
        authUser.nom
      );
    } catch (notifErr) {
      logger.warn('users.controller', '⚠️ Notification utilisateur non envoyée:', notifErr.message);
    }
  } catch (error) {
