...
exports.createMouvementStock = async (req, res) => {
  ...
  try {
    ...
    console.error(error);     
    res.status(500).json({ message: "Erreur lors de la création du mouvement", error: error.message });
  }
};
...
exports.getAllMouvementsStock = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Erreur lors de la récupération des mouvements", error: error.message });
  }
};
...
exports.getMouvementStockById = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Erreur lors de la récupération du mouvement", error: error.message });
  }
};
