...
export class PaiementsService {
  ...
  /** TODO: Ajouter pagination - récupère tous les paiements sans limite */
  ...
  getByStructure(code_structure: string): Observable<Paiement[]> {
    return this.http.get<Paiement[]>(`${this.apiUrl}/structure/${code_structure}`, {}).pipe(
  ...
  getByClientsStructure(code_structure: string): Observable<Paiement[]> {
    return this.http.get<Paiement[]>(`${this.apiUrl}/structure/${code_structure}/clients`, {}).pipe(
      tap(() => this.logger.info(`Paiements récupérés pour la structure ${code_structure}`)),
      catchError((error) => this.handleError(error, `Erreur lors du chargement des paiements de la structure ${code_structure}`))
    );
  }
  ...
  getByFournisseursStructure(code_structure: string): Observable<Paiement[]> {
    return this.http.get<Paiement[]>(`${this.apiUrl}/structure/${code_structure}/fournisseurs`, {}).pipe(
      tap(() => this.logger.info(`Paiements récupérés pour la structure ${code_structure}`)),
      catchError((error) => this.handleError(error, `Erreur lors du chargement des paiements de la structure ${code_structure}`))
    );
  }

  private getPaiementsAvecPagination(code_structure: string, endpoint: string, filter: PaiementsFilter = {}): Observable<PaiementsResponse> {
    ...
    return this.http.get<PaiementsResponse>(`${this.apiUrl}/structure/bis/${code_structure}/${endpoint}`, {
      params
    }).pipe(
      tap(() => this.logger.info(`Paiements ${endpoint} récupérés pour la structure ${code_structure}`)),
      catchError((error) => this.handleError(error, `Erreur lors du chargement des paiements`))
    );
  }

  getPaiementsClients(code_structure: string, filter: PaiementsFilter = {}): Observable<PaiementsResponse> {
    return this.getPaiementsAvecPagination(code_structure, 'clients', filter);
  }

  getPaiementsFournisseurs(code_structure: string, filter: PaiementsFilter = {}): Observable<PaiementsResponse> {
    return this.getPaiementsAvecPagination(code_structure, 'fournisseurs', filter);
  }
  /** TODO: Remplacer par un intercepteur HTTP global (voir HttpInterceptor) */
  private handleError(error: unknown, message: string) {
    return handleApiError(this.logger, 'PaiementsService', error, message);
  }
}
