...
export class RapportsFinanciersComponent implements OnInit {
  ...
  private getDonneesPourPeriode(decalage: number): {
    ...
    const depenses = this.allDepenses.filter(
      (d) =>
    ...
    const recettes = this.allRecettes.filter(
      (r) =>
  ...
  creerGraphiqueEvolution(): void {
    ...
    if (ctx) {
      this.evolutionChart = new Chart(ctx, {
        type: 'line',
        data: {
          labels: this.getDatesBetween(new Date(this.dateDebut), new Date(this.dateFin)).map((d) =>
            this.formatDateForChart(d),
          ),
          datasets: [
            {
  ...
  async exportToExcel() {
    ...
    const indicators = [
      ...
      [
        'Évolution CA',
        `${this.evolutionCA.pourcentage}% ${this.evolutionCA.tendance === 'hausse' ? '↑' : '↓'}`,
      ],
    ];
    ...
    transactionsSheet
      .getRow(transactionsSheet.rowCount)
      .eachCell((cell) => Object.assign(cell.style, headerStyle));
  ...
  getOccurenceDepensesByCategory(id: number): { montantTotal: number; occurrences: number } {
    const depensesFiltrees = this.filteredDepenses.filter(
      (d) =>
        d.categoryId === id &&
        this.resetTime(new Date(d.date)) >= this.resetTime(new Date(this.dateDebut)) &&
        this.resetTime(new Date(d.date)) <= this.resetTime(new Date(this.dateFin)),
  ...
  getOccurenceRecettesByCategory(id: number): { montantTotal: number; occurrences: number } {
    const depensesFiltrees = this.filteredRecettes.filter(
      (d) =>
        d.categoryId === id &&
        this.resetTime(new Date(d.date)) >= this.resetTime(new Date(this.dateDebut)) &&
        this.resetTime(new Date(d.date)) <= this.resetTime(new Date(this.dateFin)),
  ...
  private getDonneesJournalieres(type: 'DEPENSE' | 'RECETTE'): number[] {
    ...
    return dates.map((date) => {
      return source
        .filter(
          (item) =>
            this.resetTime(new Date(item.date)).getTime() === this.resetTime(date).getTime(),
        )
  ...
  onSearchChange(typeSearch: string): void {
    ...
    if (typeSearch === 'depense') {
      this.filteredDepenses = this.allDepenses.filter(
        (dep) =>
      ...
      this.filteredRecettes = this.allRecettes.filter(
        (rec) =>
