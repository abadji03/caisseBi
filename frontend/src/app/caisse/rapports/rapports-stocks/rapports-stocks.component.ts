...
export class RapportsStocksComponent implements OnInit {
  ...
  selectedMagasin = 'Toutes les succursales';
  categorie = 'all';
  statut = 'all';
  selectedMagasinId = -1; // Pour les administrateurs
  isAdmin = true; // Simuler un utilisateur admin, à remplacer par un réel contrôle d'accès
  isPrinting = false;
  ...
  createRepartitionChart(): void {
    ...
    const filteredCategories = Array.from(categories.entries()).filter(([_, value]) => value > 0);
  ...
  get getListeProduitsAReapprovisionner() {
    ...
    return (
      this.stocks?.filter((stock) => stock.quantiteDisponible <= stock.seuilReapprovisionnement) ||
      []
    );
  }
  ...
  async exportToExcel() {
    ...
    summarySheet.addRow([
      ...
      'Dakar, Sénégal',
      'Période',
      `${new Date(this.dateDebut).toLocaleDateString()} au ${new Date(this.dateFin).toLocaleDateString()}`,
    ]);
    summarySheet.addRow([
      'Succursale',
      this.selectedMagasinId !== -1
        ? this.getNomMagasinsById(this.selectedMagasinId)
    ...
    const indicators = [
      ...
      [
        'Valeur achat du Stock initial',
        `${this.valeurTotaleStockAchatInitial?.toLocaleString()} F CFA`,
      ],
      [
        'Valeur achat du Stock final',
        `${this.valeurTotaleVenteStockAchatFinal?.toLocaleString()} F CFA`,
      ],
  ...
  getDateUpdateById(produitId: number): Date {
    return (
      this.stocks.find((p) => p.produitId === produitId)?.dateDerniereMiseAJour ??
      new Date(1900, 9, 19)
    );
  }
  ...
  onSearchChange(typeSearch: string): void {
    ...
    if (typeSearch === 'stock') {
      ...
      this.filteredMouvements = this.allMouvements.filter(
        (prod) =>
          this.removeAccents(prod.typeMouvement?.toLowerCase()).includes(search) ||
          this.removeAccents(this.getNomProduitById(prod.produitId)?.toLowerCase()).includes(
            search,
          ),
      );
  ...
  updatefilteredTable(): void {
    this.filteredProduits = this.allProduits.slice(
