...
import { magasins, produits, stocks,mouvements, paniers, clients, vendeurs, modesPaiement } from '../../../modeles/donnees_fictives';
...
export class RapportsVentesComponent implements OnInit {
      ...
      calculerIndicateurs(): void {
        ...
        this.evolutionCA = this.calculerEvolution(this.chiffreAffairesTTC, periodePrecedente.chiffreAffairesTTC);
      ...
      getDonneesPeriodePrecedente(): { totalVentes: number, chiffreAffairesTTC: number } {
        ...
        const ventesPrecedentes = this.allVentes.filter(v =>
      ...
      calculerEvolution(valeurActuelle: number, valeurPrecedente: number): { valeur: number, tendance: '↑' | '↓' | '→' } {
      ...
      creerGraphiqueEvolutionVentes(): void {
        ...
        const dataCA = dates.map(d =>
          this.filteredVentes
            .filter(v => this.resetTime(new Date(v.dateCreation)).getTime() === this.resetTime(d).getTime())
            .reduce((sum, v) => sum + v.totalTTC, 0)
        );
        const dataVolume = dates.map(d =>
          this.filteredVentes
            .filter(v => this.resetTime(new Date(v.dateCreation)).getTime() === this.resetTime(d).getTime())
            .length
        );
      ...
      private calculerStatsVendeur(vendeurId: number): void {
        ...
        const margeBeneficiaire = ventesVendeur.reduce((sum, v) => {
          return sum + v.articles.reduce((s, a) => s + ((a.prixVenteUnitaire || 0) - (a.prixAchatUnitaire || 0)), 0);
        }, 0);
        ...
        const panierMoyen = totalVentes > 0 ?
          ventesVendeur.reduce((sum, v) => sum + v.articles.length, 0) / totalVentes : 0;
      ...
      private calculerVentesParJour(ventes: Panier[]): any[] {
        ...
        return dates.map(d => {
          const ventesJour = ventes.filter(v =>
            this.resetTime(new Date(v.dateCreation)).getTime() === this.resetTime(d).getTime()
          );
          return {
            date: d,
      ...
      onSearchChange(): void {
        ...
        this.filteredVentes = this.allVentes.filter(v =>
          ...
          (v.clientId && this.allClients.some(c =>
            c.id === v.clientId && c.nomComplet.toLowerCase().includes(search)))
        );
        this.currentPage = 1;
        this.calculerIndicateurs();
        this.calculerTopListes();
...
genererComparaison(): void {
  ...
  try {
    ...
    this.comparaisonData = {
      ...
      ticketMoyen1: data1.totalVentes > 0 ? (data1.chiffreAffairesTTC || 0) / data1.totalVentes : 0,
      ticketMoyen2: data2.totalVentes > 0 ? (data2.chiffreAffairesTTC || 0) / data2.totalVentes : 0
    };
...
private getDonneesPourPeriode(periode: string): {data: any, label: string} {
  ...
  const ventes = this.allVentes.filter(v => {
    ...
    return dateVente >= this.resetTime(startDate) &&
...
async exportToExcel() {
  ...
  const indicators = [
    ...
    ['Chiffre d\'Affaires', `${this.chiffreAffairesTTC.toLocaleString()} F CFA`, `${this.chiffreAffairesHT.toLocaleString()} F CFA HT`],
    ['Marge bénéficiaire', `${this.margeBeneficiaire.toLocaleString()} F CFA`, `${(this.margeBeneficiaire / this.chiffreAffairesHT * 100).toFixed(2)}%`],
    ['Ticket moyen', `${this.ticketMoyen.toLocaleString()} F CFA`, `${this.panierMoyen.toFixed(1)} produits/vente`],
    ['Évolution CA', `${this.evolutionCA.valeur}%`, this.evolutionCA.tendance === '↑' ? 'Hausse' : this.evolutionCA.tendance === '↓' ? 'Baisse' : 'Stable'],
    ['Évolution volume', `${this.evolutionVolume.valeur}%`, this.evolutionVolume.tendance === '↑' ? 'Hausse' : this.evolutionVolume.tendance === '↓' ? 'Baisse' : 'Stable']
  ];
  ...
  paymentSheet.addRow(['Mode', 'Montant (F CFA)', 'Transactions', 'Pourcentage']).eachCell(cell => Object.assign(cell.style, headerStyle));
  ...
  this.statmodesPaiement.forEach((mode: { mode: string; montantTotal: number; occurrences: number }) => {
    paymentSheet.addRow([
      mode.mode,
      mode.montantTotal,
      mode.occurrences,
  ...
  productsSheet.addRow([
    'Produit', 'Quantité', 'Prix de vente', 'Transactions', 
    'CA TTC', 'Marge', '% Marge'
  ...
  clientsSheet.addRow([
    'Client', 'Transactions', 'CA TTC', 'Dernière visite', 'Ticket moyen'
  ]).eachCell(cell => Object.assign(cell.style, headerStyle));
  ...
  sellersSheet.addRow([
    'Vendeur', 'Ventes', 'CA HT', 'CA TTC', 'Ticket moyen', 'Panier moyen'
  ]).eachCell(cell => Object.assign(cell.style, headerStyle));
  ...
  salesSheet.addRow([
    'Date', 'N° Ticket', 'Client', 'Articles', 'Total TTC', 'Paiement', 'Vendeur', 'Statut'
  ]).eachCell(cell => Object.assign(cell.style, headerStyle));

  this.filteredVentes.forEach(vente => {
    const paiements = vente.paiements?.map(p => 
      `${this.getNomModePaiement(p.methodePaiement)}: ${p.montant.toFixed(0)} F CFA`
    ).join('\n') || 'Non spécifié';
    ...
    salesSheet.addRow([
      ...
      if (colNumber !== 6) { // Ne pas appliquer le style à la colonne des paiements (qui a un style spécial)
