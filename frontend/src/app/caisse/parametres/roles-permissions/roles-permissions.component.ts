      ...
      this.rolePermissionsService.updateRole(this.currentRoleId, roleData)
        .subscribe({
          next: () => {
            ...
            this.rolePermissionsService.updatePermissionsForRole(this.currentRoleId!, permissionIds)
              .subscribe({
                next: () => {
                  this.loadRoles();
                  this.closeModal();
      ...
      this.rolePermissionsService.createRole(roleData)
        .subscribe({
          next: (newRole) => {
            ...
            this.rolePermissionsService.updatePermissionsForRole(newRole.id!, permissionIds)
              .subscribe({
                next: () => {
                  this.loadRoles();
                  this.closeModal();
      ...
      .subscribe({
        next: (rolePermissions) => {
          ...
          if (permissionIds.length > 0) {
            //console.log('Succés');
            this.rolePermissionsService.removePermissionsFromRole(roleId, permissionIds)
              .subscribe({
                next: () => {
...
private deleteRoleFinally(roleId: number): void {
  ...
  if (isChecked) {
    ...
    this.roleForm.get('permissions')?.setValue(permissions.filter((id: number) => id !== permissionId));
  }
}
}