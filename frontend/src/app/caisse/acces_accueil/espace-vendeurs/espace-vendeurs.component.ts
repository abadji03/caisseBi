...
export class EspaceVendeursComponent implements OnInit {
  ...
  menu: string = 'Accueil';
  subMenu: string = 'Tableau de bord';
  ...
  currentMenu: string = 'Accueil';
  currentSubMenu: string = 'Vue d\'ensemble';
  fullTitle: string = '';
 ...
 ngOnInit(): void {
  ...
  updateTitles(): void {
    ...
    else if (url.includes('entrees-sorties') || url.includes('stock') || 
             url.includes('catalogue-produits') || url.includes('enregistrement-produit')) {
    ...
    else if (url.includes('magasins') || url.includes('gerant') || 
