...
export class RapportsVentesComponent implements OnInit {
      ...
      // Méthodes d'export (similaires à celles du rapport financier)
      async exportToPDF() {
        // Implémentation similaire à celle du rapport financier
      }
      ...
      // Méthode pour obtenir les options de comparaison en fonction du type sélectionné
/*     getComparisonOptions(): {value: any, label: string}[] {
      switch (this.comparaisonType) {
        ...
        case 'vendeur':
          return this.vendeurs.map(v => ({
            value: v.id,
            label: v.nom
          }));
        case 'magasin':
          return this.magasins.map(m => ({
            value: m.id,
            label: m.nom
          }));
        default:
